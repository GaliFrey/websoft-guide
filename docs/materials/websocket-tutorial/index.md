...
Для тестирования WebSocket на клиенте будем использовать сгенерированную ИИ <a href="./ws-client/" target="_blank" rel="noopener">HTML страницу</a>. Создадим настраиваемый шаблон, скопируем туда код страницы и будем использовать её для тестирования.

Ссылка на такой шаблон будет выглядеть так:
```
<протокол>://<адрес сервера>/_wt/doc_type/custom_web_template_id/<ID настраиваемого шаблона>
```

!!! warning "Важно"
	Для подключения по `wss` необходим протокол `HTTPS`, для подключения по `ws` - `HTTP`. Если на DEV сервере нет сертификата, то для подключения используем `ws`, на проде обязательно используем `wss`
...
```
wss://domain.ru/services/main_ws_service
```
...
```
wss://domain.ru/services/main_ws_service?X-StatefulSocketId=123
```
...
```
wss://domain.ru/services/main_ws_service?X-StatefulSocketId=id:10001::group_id:10001
wss://domain.ru/services/main_ws_service?X-StatefulSocketId=id:10002::group_id:10001
wss://domain.ru/services/main_ws_service?X-StatefulSocketId=id:10003::group_id:10002
wss://domain.ru/services/main_ws_service?X-StatefulSocketId=id:10004::group_id:10002
```
...
#### Вариант для 1.22.10.29 (434 сборка)
...
* будет создан ключ хранилища `sUserDataKey` со значением `ws_group_recipients`
...
#### Вариант для 1.24.4.27 (906 сборка)
...
Запустим агент для отправки уведомлений первым двум подключениям. Обратите внимание на способ фильтрации `["$socket_type:ws_group", true]`
